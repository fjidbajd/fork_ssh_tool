                    ...
                    clear
                        ...
                        until [[ -z $(ss -tunlp | grep -w udp | awk '{print $5}' | sed 's/.*://g' | grep -w "$new_port") ]]; do
                            if [[ -n $(ss -tunlp | grep -w udp | awk '{print $5}' | sed 's/.*://g' | grep -w "$new_port") ]]; then
                                echo -e $'\033[1;91m${vport}端口已经被其他程序占用，请更换端口重试！\033[0m'
                                read -p $'\033[1;35m设置Hysteria2端口[1-65535]（回车跳过将使用随机端口）：\033[0m' new_port
                                [[ -z $new_port ]] && new_port=$(shuf -i 2000-65000 -n 1)
                            fi
                        done
            ...
            case $sub_choice in
                ...
                3)
                    clear
                        ...
                        until [[ -z $(ss -tunlp | grep -w udp | awk '{print $5}' | sed 's/.*://g' | grep -w "$new_port") ]]; do
                            if [[ -n $(ss -tunlp | grep -w udp | awk '{print $5}' | sed 's/.*://g' | grep -w "$vnew_port") ]]; then
                                echo -e $'\033[1;91m${new_port}端口已经被其他程序占用，请更换端口重试！\033[0m'
